...
public class CsAnnounceService {
    ...
    //공지 사항 조회 (목록 / 세부)
    public List<CsAnnounceBean> getAnnounce(Integer announce_id) { //공지 ID를 받은 경우 공지 내역 상세를 리스트로.
        ...
        } else { //공지 ID를 받지 않은 경우에는 공지 내역 목록을 리스트로.
            List<CsAnnounceBean> announceList = csMapper.getAnnounce();
            return announceList;
        }
    }
    ...
    public List<CsAnnounceBean> getAnnounceByCategory(CsAnnounceBean.announce_category announce_category){
        return csMapper.getAnnounceByCategory(announce_category);
    }
